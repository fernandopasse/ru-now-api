...
//Busca dados CardapioRU
app.get('/dataRU', (request, response) => {
  requestFunc(url, (error, res, html) => {
      if(!error){
          ...
          for (var i = 1; i <= daysToFetch; i++) {
            ...
            var lunchMeat = $(day + ' .almoco .descricaoAlmoco .segundaLinha .descricaoCarne .descricaoRefeicao').text()
            var lunchGuarnicao = $(day + ' .almoco .descricaoAlmoco .segundaLinha .descricaoGuarnicao .descricaoRefeicao').text()
            var lunchSalad = $(day + ' .almoco .descricaoAlmoco .segundaLinha .descricaoSalada .descricaoRefeicao').text()
            data.push({
              date: date,
              lunch: {meat: lunchMeat, guarnicao: lunchGuarnicao, salad: lunchSalad}
            })
          }
...
app.get('/dataMU', (request, response) => {
  requestFunc(url, (error, res, html) => {
      if(!error){
          const $ = cheerio.load(html)
          const daysToFetch = $('#refeicoesMU').attr('quantidademax')
          const data = []
          for (var i = 1; i <= daysToFetch; i++) {
            var day = '#MUdia' + i
            var date = $(day).attr('name')
